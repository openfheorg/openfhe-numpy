...
# the network's operation. In this scenario, the input data is encrypted
# and the model's weights are unencrypted (plaintext). The purpose is to show 
# how to perform secure inference using OpenFHE-Numpy, where the computations 
# are done on encrypted data with plaintext weights and biases.
...
def onp_forward_pass(X, W1, b1, W2, b2, cc, keys, batch_size):
    ...
    onp.generate_slicing_key(keys.secretKey, onp_ct_X.original_shape)
    # Plaintext weights W1 and biases b1
    ...
    onp_ct_Z1 = onp_ct_X @ onp_pt_W1 + onp_pt_b1
    ...
    # Output layer
    # Plaintext weights W2 and biases b2
    ...
    onp_ct_Z2 = onp_ct_A1 @ onp_pt_W2 + onp_pt_b2
    ...
    onp_ct_A2_masked = onp_ct_A2[0, 0]
    decrypted_A2_masked = onp_ct_A2_masked.decrypt(keys.secretKey, unpack_type="original")
    
    print(f"Evaluating ONP Forward Pass (Encrypted) Ended Successfully\n")
    
    return np.array([[decrypted_A2_masked]])
