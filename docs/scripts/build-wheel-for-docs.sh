...
# build an OpenFHE-numpy wheel on readthedocs (RTD) and print the full wheel path.
# logs go to stderr; the wheel path goes to stdout so the caller can capture it.
...
# the real branch name checked out by RTD (NOT "latest"/"stable")
...
cd "openfhe-numpy-packager"
git checkout main

# override the tag/version selector in ci-vars.sh to the RTD branch name.
sed -i "s|^OPENFHE_NUMPY_TAG=.*|OPENFHE_NUMPY_TAG=${REAL_BRANCH_NAME}|" ci-vars.sh
...
printf '%s\n' "${WHEEL_BUILD_DIR}/openfhe-numpy-packager/${WHEEL_DIR}/${matches[0]}" >&3
